...
export async function loadAgentMemoryLayers(
  ...
  const identityDocs = documents.filter((document) => isIdentityFile(document.path, document.scope));
...
function packTaggedBlock(
  ...
  const packed = packMemoryLayers(documents, Math.max(0, maxBytes - byteLength(preamble) - byteLength(closing)));
