...
export function releaseExtraDisplayCommand(screenKey: string, leaseId?: string): string {
  ...
  return [
    ...
    'if [ "$index" -ne 0 ]; then display_number=$((index + 1)); view_port=$((6080 + index * 2)); control_port=$((6081 + index * 2)); view_vnc_port=$((5900 + index * 2)); control_vnc_port=$((5901 + index * 2)); pkill -f "Xvfb :$display_number -screen" || true; pkill -f "HOME=/tmp/fluxbox-home-$display_number DISPLAY=:$display_number fluxbox" || true; pkill -f -- "chromium-screen-$display_number" || true; pkill -f "(^|/)x11vnc .* -rfbport $view_vnc_port" || true; pkill -f "(^|/)x11vnc .* -rfbport $control_vnc_port" || true; pkill -f "^/usr/bin/python3 .*websockify.*$view_port" || true; pkill -f "novnc_proxy.*--listen $view_port" || true; pkill -f "^/usr/bin/python3 .*websockify.*$control_port" || true; pkill -f "novnc_proxy.*--listen $control_port" || true; pkill -f "autocutsel -display :$display_number -selection" || true; rm -f "/tmp/.X$display_number-lock" "/tmp/.X11-unix/X$display_number" "/tmp/rakazo/control-token-$display_number" "/tmp/rakazo/view-password-$display_number" "/tmp/rakazo-view-$display_number.vncpass" "/tmp/rakazo-control-$display_number.vncpass" "/tmp/rakazo/screen-$display_number.lock"; fi',
    'rm -f "$slot"',
...
export function ensureExtraDisplayCommand(
  ...
  return [
    ...
    `  if command -v autocutsel >/dev/null 2>&1 && ! pgrep -f 'autocutsel -display ${layout.display} -selection' >/dev/null 2>&1; then autocutsel -display ${layout.display} -selection CLIPBOARD -fork || true; autocutsel -display ${layout.display} -selection PRIMARY -fork || true; fi`,
