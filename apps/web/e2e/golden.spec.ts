...
test("takeover, routine, plugins, and export are reachable", async ({ page }, testInfo) => {
  ...
  await expect(featured).toContainText(
    /Gmail[\s\S]*Google Calendar[\s\S]*Google Drive[\s\S]*Slack[\s\S]*GitHub[\s\S]*Notion[\s\S]*Linear/,
  );
  await expect(featured.getByText("Search, read, draft, and manage email.")).toBeVisible();
  ...
  await expect(
    advanced.getByText(/MCP · https:\/\/treg\.to\/mcp\/ · credential saved/),
  ).toBeVisible();
  ...
  await page.getByRole("button", { name: "Verify and add", exact: true }).click();
  await expect(
    advanced.getByText(/MCP · https:\/\/mcp\.example\.test\/mcp · no auth/),
  ).toBeVisible();
  ...
  await page.getByRole("button", { name: "Verify and add", exact: true }).click();
  await expect(
    advanced.getByText(/API · https:\/\/api\.example\.test\/v1 · credential saved/),
  ).toBeVisible();
